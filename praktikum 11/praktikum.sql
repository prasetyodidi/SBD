...
BEGIN
    ...
    DECLARE total INT;
    DECLARE pelangganId VARCHAR(40);
    DECLARE cursor1 CURSOR FOR SELECT kd_produk, sum(jumlah) AS 'total' FROM tb_detail_jual GROUP BY kd_produk ORDER BY count(kd_produk) DESC LIMIT 3;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET exitLoop = TRUE;
    ...
    LOOP
        FETCH cursor1 INTO pelangganId, total;
        SELECT kd_produk, count(kd_produk) AS 'total sale' FROM tb_detail_jual GROUP BY kd_produk ORDER BY count(kd_produk) DESC LIMIT 3;
        IF exitLoop THEN
            CLOSE cursor1;
            LEAVE ulang;
        END IF;
